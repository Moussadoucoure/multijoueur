   class PersonnageBleu{
    ...
    avancer(secondeEcoulee){
        ...
        this.spritePersonnage.x = this.limiterMouvement(this.spritePersonnage.x + PersonnageBleu.VITESSE_PIXEL_SECONDE * secondeEcoulee, this.spritePersonnage.y).x;
    }
    ...
    this.spritePersonnage.y = this.limiterMouvement(this.spritePersonnage.x, this.spritePersonnage.y - PersonnageBleu.VITESSE_PIXEL_SECONDE * secondeEcoulee).y;
  }
   ...
   sauter(secondeEcoulee){
    ...
    if(tempsActuel- this.debutSaut < PersonnageBleu.DUREE_SAUT / 2){
      this.monter(secondeEcoulee);

    }else{
      this.descendre(secondeEcoulee)
    }
    if(tempsActuel - this.debutSaut >= PersonnageBleu.DUREE_SAUT){
      this.debutSaut = 0;
      this.etat = PersonnageBleu.ETAT.ATTENTE;
    }
